...
function buildWrappedPython(code, functionName, args, argTypes, beatcodeId) {
  ...
  const isMergeKLists = fnLower === "mergeklists";
...
function buildWrappedJavascript(code, functionName, args, argTypes, beatcodeId) {
  ...
  if (!useLinkedList && !useTree) {
    return `${code}\n\n(async () => {\n  try {\n    let _args = JSON.parse(JSON.stringify(${JSON.stringify(args || [])}));\n    function __breakPotentialCycle(head) {\n      if (!head || typeof head !== "object") return head;\n      const seen = new Set();\n      let cur = head;\n      let prev = null;\n      let guard = 0;\n      while (cur && typeof cur === "object" && guard < 200000) {\n        if (seen.has(cur)) {\n          if (prev && typeof prev === "object") prev.next = null;\n          break;\n        }\n        seen.add(cur);\n        prev = cur;\n        cur = cur.next;\n        guard += 1;\n      }\n      return head;\n    }\n    if (!${isHasCycle ? "true" : "false"} && !${isMergeKLists ? "true" : "false"}) {\n      _args = _args.map((a) => __breakPotentialCycle(a));\n    }\n    const __targetName = ${JSON.stringify(functionName)};\n    const __resolveCallable = (name) => {\n      if (typeof globalThis[name] === "function") return globalThis[name];\n      try {\n        const localFn = eval(name);\n        if (typeof localFn === "function") return localFn;\n      } catch (_) {}\n      if (typeof Solution === "function") {\n        try {\n          const inst = new Solution();\n          if (inst && typeof inst[name] === "function") return inst[name].bind(inst);\n        } catch (_) {}\n      }\n      return undefined;\n    };\n    const __namedFn = __resolveCallable(__targetName);\n    const __fallbackFn = __resolveCallable("getIntersectionNode");\n    const __invoke = __namedFn || __fallbackFn;\n    if (typeof __invoke !== "function") {\n      throw new Error('Function "' + __targetName + '" not found in submitted code');\n    }\n    const _result = await Promise.resolve(__invoke(..._args));\n    process.stdout.write(JSON.stringify({ __judge: { result: _result, mutatedArgs: _args } }) + "\\n");\n  } catch (err) {\n    process.stderr.write(String(err) + "\\n");\n    process.exit(1);\n  }\n})();\n`;
  }
...
function buildWrappedCpp(code, functionName, args, argTypes) {
  ...
  const isMergeKLists = normalizedLower === "mergeklists";
  ...
  const declaredArgs = safeArgs.map((arg, idx) => {
    ...
    if (usesListNode && /\bvector\s*<\s*ListNode\s*\*\s*>/.test(normalizedHint) && Array.isArray(arg)) {
      return `vector<ListNode*> __arg${idx} = __make_list_array(vector<vector<int>>${serializeCppLiteral(arg, "vector<vector<int>>")});`;
    }
  ...
  const listNodeHelpers = usesListNode ? [
    ...
    "  return dummy.next;",
    "}",
    "vector<ListNode*> __make_list_array(const vector<vector<int>>& groups) {",
    "  vector<ListNode*> out;",
    "  out.reserve(groups.size());",
    "  for (const auto& g : groups) out.push_back(__make_list(g));",
    "  return out;",
    "}",
  ...
  return [
    ...
    "string __to_json(const char& value) { return __json_escape(string(1, value)); }",
    "string __to_json(const signed char& value) { return __json_escape(string(1, static_cast<char>(value))); }",
    "string __to_json(const unsigned char& value) { return __json_escape(string(1, static_cast<char>(value))); }",
    "template <typename T, typename std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<T, bool>, int> = 0>",
    "string __to_json(const T& value) {",
    "  if constexpr (std::is_signed_v<T>) return to_string((long long)value);",
